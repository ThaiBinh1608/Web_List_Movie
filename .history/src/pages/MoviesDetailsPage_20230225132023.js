...
const MoviesDetailsPage = () => {
    ...
    return (
        <>
          ...
          {genres.lenght > 0 && yield <div className="flex itemss-center gap-x-5 mb-10">
              {genres.map(item => (
                <span className='py-2 px-4 border-primary text-primary border' key={item.id} >{item.name}</span>
              ))}
