...
const TVseriesDetailsPage = () => {
  ...
  const {data} = useSWR(tmdbAPI. getTVDetails(), fetcher);
  console.log(data);
  if (!data) return null;
...
function TVVideo() {
const { tvId } = useParams();
const { data } = useSWR(`https://api.themoviedb.org/3/tv/${tvId}/videos?api_key=95f2419536f533cdaa1dadf83c606027`, fetcher);
if (!data) return null;
const { results } = data;
...
function TVSimilar() {
const { tvId } = useParams();
const { data } = useSWR(`https://api.themoviedb.org/3/tv/${tvId}/similar?api_key=95f2419536f533cdaa1dadf83c606027`, fetcher);
//https://api.themoviedb.org/3/movie/{movie_id}/credits?api_key=
if (!data) return null;
const { results } = data;
if (!results || results.length <= 0) return null;
