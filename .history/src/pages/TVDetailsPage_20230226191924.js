...
import TVCard from "../components/TVseries/TVCard";
...
function TVCredits() {
  const { TVId } = useParams();
  const { data } = useSWR(tmdbAPI.getTVMeta(TVId, "credits"), fetcher);
...
function TVVideo() {
  ...
  const { data } = useSWR(tmdbAPI.getTVeMeta(movieId, "videos"), fetcher);
  if (!data) return null;
  const { results } = data;
...
function TVSimilar() {
  const { TVId } = useParams();
  const { data } = useSWR(tmdbAPI.getTVMeta(TVId, "similar"), fetcher);
  //https://api.themoviedb.org/3/movie/{movie_id}/credits?api_key=
  if (!data) return null;
  const { results } = data;
  if (!results || results.length <= 0) return null;
